...
def get_supporter_settings(event, context):
    ...
    block['employer'] = settings[0][0].get('stringValue')
    block['title'] = settings[0][1].get('stringValue')
    block['team_name'] = settings[0][2].get('stringValue')
    block['feedback'] = settings[0][3].get('stringValue')
    block['office'] = settings[0][4].get('stringValue')
    block['google_doc_link'] = settings[0][5].get('stringValue')
    ...
    if(settings == []):
        return {
            'body': json.dumps("Link could not be found"),
            'statusCode': 404
        }
