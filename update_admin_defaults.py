...
def update_admin_defaults(event,context):
    ...
    medium = event['medium_of_appt']
    max_students = event['max_students_for_specialization']
    duration = event['duration_for_specialization']
    hours = event['hours_before_appointment_for_supporter']

    sql = 'UPDATE default_table SET medium_of_appt =:medium, \
    ...
    sql_parameters = [{'name':'medium', 'value' : {'longValue' : medium}},
